-- ============================================================================
-- fn_market_comps: expose `certified` per comp.
--
-- The CPO premium needs to split CERTIFIED from NON-certified WITHIN the used
-- pool — a certified car's `condition` is still 'used', so the boolean is the
-- only thing that distinguishes it. This adds `certified` to each returned row.
-- Additive: computeBand and every existing caller read named fields and ignore
-- the new one, so nothing else changes. (Same signature -> a plain replace.)
-- ============================================================================
