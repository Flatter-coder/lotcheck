...
import { computeReconciliation, computeFinancingTrap, buildCounterScript, hasTrustedFinanceRate, TRUSTED_APR_SOURCES } from "../_shared/deal.ts";
...
import { aprClaim, type AprBenchmark } from "../_shared/apr-reference.ts";
import { msrpIsPresentTense, applyConditionToMsrp } from "../_shared/msrp-basis.ts";
import { applyAllInResolution } from "../_shared/jurisdiction.ts";
...
import { powertrainCompatible, stripPowertrain, baseNameplate } from "../_shared/model-identity.js";
...
const CACHE_VER = "2026-09-25b";  // 25b: canonical v16 seals marketValue.pool -- every listing the report's shortlist page walks through (the kept set, the price outliers, the mileage window's leavers, each tagged with why) and how many other trims were set aside. THE BUMP IS MANDATORY: report-auth.ts recomputes canonicalReport() from the submitted body, so a cached v15 analysis replayed after this deploy would be refused as signature_mismatch.  // 25a: the counter-script asks for the bank's buy rate before the buyer discloses theirs. A finance manager states on tape that he prices just under whatever rate a buyer discloses first, not against a fixed spread over the buy rate -- disclosing your own pre-approval hands him the exact number to shave. The existing Rate move needs a quoted dealer rate AND a published promo rate to compare; this fires on the emptiest possible analysis, before either exists, same as the always-on "In writing" and "Bill of sale" closers. No number of its own -- it only asks a question, so there is nothing to get wrong. THE BUMP IS MANDATORY: counterScript.moves is computed server-side and STORED in the analysis, so a replayed cache would keep showing the old move list without this line after the fix deployed.  // 24c: freight and PDI now answers for 25 makes and by MODEL YEAR. freightFor() takes the listing's year and answers only that year's published figure (VW prices the 2026 Atlas at $2,250 and the 2027 at $2,450; Ford's 2025-to-2026 figures rose $100-$400), so a 2027 car is no longer measured against a 2026 charge. A maker figure for destination that does not say it includes PDI (Porsche, Ford, Lincoln, Stellantis, Rivian, Lucid, GM) is named and never compared to a listing's Freight-and-PDI line -- it previously could raise point 03 by the PDI. BMW gains $3,470 on every line from its own Alberta price list; Ram 1500's $2,195, contradicted by Stellantis's own $2,595, is gone. freightLine output is STORED in the analysis (point 03), so a replayed cache would keep the old comparison.  // 24b: canonical v15 seals marketValue.rows -- up to seven of the listings behind the market figures (price, km, year, trim, city, dealer, read date), closest in odometer to the subject -- so the redesigned report prints its compare and shortlist pages from the SEALED record instead of a second read at PDF time. THE BUMP IS MANDATORY: report-auth.ts recomputes canonicalReport() from the submitted body, so a cached v14 analysis replayed after this deploy would be refused as signature_mismatch. // 24a: the comps now come from fn_comp_pool (20260924_comps_attribution.sql). A dealer group's site -- jpautogroup.com, 2,800 units filed as "AUDI EDMONTON NORTH" -- no longer enters the value band, the count line or the named comps; a car two dealers both list counts once and prints no dealer name; and the dealer count is keyed on dealerKey and prints no number when any row's dealer is unknown. THE BUMP IS MANDATORY: marketValue, marketCount and the named comps are STORED in the analysis, so a replayed cache would keep the double-counted band and the wrong dealer beside real prices. // 18m: this value can now be READ BACK from the running function, and the deploy fails when it does not match. A GET on analyze-listing-url returns {cacheVer} and nothing else -- no body read, no vendor call, no credit hold, no database round trip -- and deploy-edge-functions.yml compares it against the value committed here, failing the run on any mismatch, on any non-200, and on a response that carries no cacheVer at all. Until now that workflow deployed, read the CLI exit code, and reported success: exit 0 says the CLI finished, not which bundle the gateway serves. Every entry in this comment marked THE BUMP IS MANDATORY is keyed on exactly this string -- `_cacheVer !== CACHE_VER` at the cache read is the only thing that makes a stored analysis re-scan -- so a deploy that silently shipped an older bundle left every cached report replaying the pre-fix claim behind a green check, which is "Green signal, no check", the first shape named in docs/FIXING-HISTORY.md, sitting on the one step that puts code in front of buyers. THIS PARTICULAR BUMP CHANGES NOTHING A REPORT SAYS, and it is not free: it discards every cached analysis, so each previously-checked URL re-runs the full paid read path on its next open. It is taken rather than exempted because check:cache-ver is blunt ON PURPOSE -- "only a health branch" is not a property a regex can prove about a diff to this file, and an exemption that cannot be proved is the hole the next real miss walks through. It also buys the one thing a first deploy of a readback cannot otherwise have: a value that is NEW, so the first green is proof the new bundle is live rather than a restatement of what was already there. // 18l: drivetrain is corroboration, never identification. "exact" is the label that authorises an over/under-MSRP figure, and a row could earn it without having identified the car at all. Two ways, both measured. FIRST, a row with NO trim name takes no penalty from any scoring rule -- the conflict rules only punish naming the WRONG grade, and a row naming none makes no competing claim -- so where the listing states a grade the catalogue does not carry, every properly named row takes -5 and the nameless row wins at 0: exact at $41,995 beside Limited $52,995 and Luxury $56,995, printing "$3,000 OVER MSRP" from a row that does not say which car it is. The single-row path had required a trim since the Mach-E fix and this branch never did, so the row was REFUSED when alone and ACCEPTED once a better-named row stood beside it. SECOND, where neither side's grade is in KEY_TOKENS no conflict rule fires at all, so drivetrain (+4) is the only thing scoring -- decided by which row happens to have its drivetrain column filled in, which 83.9% of live rows do not. A "Comfortline AWD" listing picked the Highline row at $45,995 over the Comfortline row at $38,995; 3 of 5 sweep shapes picked the wrong row and all three said exact. A THIRD was found by mutation-testing the fix rather than by the audit: the single-row path asked only whether the row HAD a name, never whether it was the name the listing states, so a lone Highline row at $38,995 against a Comfortline listing asking $41,995 printed "$3,000 OVER MSRP". The rule is not "does the row name a grade" but "did it beat one that did" -- a ladder split only by drivetrain still resolves exactly, because there is no better-named row it could have lost to. THE BUMP IS MANDATORY: msrpBasis is computed server-side and STORED, and every over/under claim, the leverage score and counter-script move S14 all key off exact, so a replayed cache would keep making the claim after the fix deployed.  // 18k: the QUOTE path stops reading "we could not tell which province" as "not an all-in province". resolveAllInAuthority returns null for three different situations -- the city did not resolve, the province resolved but does not advertise all-in, and we hold no benchmark for it -- and analyze-quote collapsed all three behind if(ai) into an absent allInPricing, which qualifyMsrpClaim reads as not-all-in and subtracts against. Measured on the Charlesglen shape with the city missing: BEFORE comparable true, delta 3164, so the report said 3,164 OVER MSRP on a dealer who had added nothing; AFTER it refuses and names what it could not settle. That is the defect that once printed 11,173 against a real gap of 8,095, and its fix landed on analyze-listing-url ONLY. Both paths now call ONE shared applyAllInResolution, so the fact has a single author by construction rather than two blocks that agree today. basisUnknown and allInPricing are computed server-side and STORED, so a replayed cache would keep the old subtraction.  // 18j: the sentence a buyer reads INSTEAD of an over/under figure stops printing a doubled article. basisRefusal templates each supply their own article and the fallback for an unnamed make was "the manufacturer", so a signed report could print "and the the manufacturer figure we hold for this trim", "The the manufacturer figure..." and "We hold a the manufacturer MSRP for this trim". Only on the unnamed-make path, which is why it survived: every fixture passed a make. Second occurrence of the same collision -- canada-terms.ts:45 records "the the provincial registry" being fixed once already. The refusal text is STORED in the analysis, so a replayed cache would keep printing it.  // 18i: a catalogue row may no longer confirm a powertrain it never stated. The fuel partition discards itself when the pool holds no row of the listing powertrain, and rowConfirmsConfig only ever asked about DRIVETRAIN -- so a hybrid listing matched a GAS row of the same trim name and came back basis exact, the label that authorises an over-MSRP accusation. Reproduced on a RAV4 Limited ladder: hybrid listing with the hybrid row MISSING returned exact at 45,990 and the report said 4,910 OVER MSRP; with the hybrid row present it returned exact at 50,900 and the gap was ZERO. Same dealer, same car -- the accusation existed only because a row was missing, which is the IONIQ 9 incident in a different column. It is not confined to genuinely-gas rows either: fuelKind(null) returns gas and 757 of 1,512 live rows carry no fuel_type, so unlabelled rows were confirming powertrains they knew nothing about. The figure is still shown as a floor; only the label is withheld, and only when the listing actually stated a powertrain. THE BUMP IS MANDATORY: msrpBasis is computed server-side and STORED, and every over/under claim, the leverage score and counter-script move S14 all key off exact, so a replayed cache would keep making the claim after the fix deployed.  // 18h: the PDF stops making an over/under-MSRP claim the on-screen report refuses. The page-1 MSRP range bar and the Price vs MSRP prose each computed their own qp - ms and drew it on msrpBasis === "exact" alone. Exact means we found the right ROW; it says nothing about whether the two figures are measured the same way. In AB/ON/BC/QC the advertised price is all-in by law while half the catalogue is ex-freight or records no basis, so on identical inputs the PDF printed "+3,164 OVER MSRP" where qualifyMsrpClaim returns -29 against the manufacturer all-in -- a 3,193 swing that FLIPS THE SIGN of the headline figure on the artifact a buyer forwards to the dealer. Both now take comparable/delta/reference from the gate, the bar rails against the claim reference rather than the ex-freight MSRP so the marks and the number agree, and an exact match that cannot be compared prints the gate refusal instead of a figure. This is the fourth author of the subtraction 13o removed from three surfaces; test:no-raw-msrp-delta now refuses a fifth. THE BUMP IS MANDATORY: the PDF is rendered from the STORED analysis, so a replayed cache would keep printing the old figure after the fix deployed.  // 18g: an inflated-sticker accusation now requires a SHARED price basis. resolveMsrpAuthority named a dealer padded on a bare stated > msrp * 1.03 && gap > 800; ref.priceBasis was received, documented and passed through, and never consulted. In AB/ON/BC/QC the advertised price is all-in by law while 758 of the 1,512 live catalogue rows are ex-freight or declare no basis, so a compliant dealer was accused by EXACTLY the freight -- BMW 68,000 ex-freight against 71,470 all-in printed overBy 3,470, which is BMW Alberta own published Freight and PDI to the dollar; GM 3,143; Toyota 2,050. Both thresholds sit below this market freight range of 2,000-4,400, so the test could not tell a padded sticker from a legally required one. 13o fixed this same class on the hero band, the gap bar and counter-script move S14 and never reached msrp-authority.js. THE BUMP IS MANDATORY: analysis.msrpInflation is computed server-side and STORED, and analyze-quote prints it verbatim in a signed report naming the dealer and the make, so a replayed cache would keep reciting the accusation after the fix deployed. A refused comparison now carries inflationRefused with the reason, so the absence is NOTED rather than rendered green.  // 18f: Freight and PDI for 19 more models, from three makers' own build-and-price APIs. freightFor() previously returned null for every Hyundai model except Tucson and for every Mazda model, so point 03 said nothing; it now answers for 11 Hyundai and 10 Mazda models and a replayed cache would keep the old silence. Mazda itemises Freight and PDE separately (1455 + 740 on a CX-5) and the report's Freight & PDI is the two together. Hyundai's electric models also gain their own dealer-fee ceiling of 599 against a brand row of 799, so a fee that read "at the cap" on an IONIQ 9 now reads 200 above what Hyundai's own configurator bills. Genesis gains an all-in basis and a captured all_in_price -- its prices are all-inclusive by its own wording, the opposite of its sibling brand -- so any surface comparing against a Genesis reference changes what it says.  // 18e: "no distance limit" is recognised as the explicit statement it is. "Unlimited" is OUR word -- Honda publishes "5-year/no distance limit", Subaru "5-year/no km limit", Lexus "regardless of distance travelled". warranty-verify.mjs had known all of those phrasings since the Lexus fix, but only when reading a MANUFACTURER'S page; reading our own stored value it knew none of them, so Honda and Subaru came back as "could not read our own stored value as a term". The vocabulary now lives in one file that both readers import, so those terms render as the unlimited distance the maker actually stated rather than as an unreadable value or an unstated one. remainingWarranty is computed server-side and SEALED into the signed report.  // 18d: "no distance stated" stops rendering as "distance unlimited". parseCoverage returned km = null for BOTH an explicit "unlimited km" and a term that mentions no distance at all, and RemainingTerm turned km == null into kmUnlimited, which the report printed as "(distance unlimited)". MINI publishes "12-year Rust Perforation Warranty" and Polestar "Coverage lasts for the first 12 years after delivery" -- neither states a kilometre limit and neither says unlimited, so the report was telling buyers something no manufacturer told us, on a rust-through line, in the direction that overstates cover. Those two rows now hold the terms as published and the line says the maker publishes no kilometre limit. remainingWarranty is computed server-side and SEALED into the signed report, so a replayed cache would keep printing the old claim.  // 18c: the report finally says something about the RATE. computeFinancingTrap only ever compared the quoted rate to a manufacturer promo INSIDE the discount-vs-subvented-financing trap, so on every other deal the rate -- where a finance manager on the record says the money is -- went unexamined. analysis.aprCheck now compares a trusted-source quoted APR against the manufacturer published rate and states the difference as DOLLARS ACROSS THE WHOLE LOAN, never as a monthly figure alone: a 0.30-point spread is "$4.25 a month" and $306 over 72 months, and only the second number makes anyone ask. It refuses rather than compares when the posted term does not match the loan term, when the reference is over 30 days old, undated, or unattributed, and when the rate itself came from anything but sm360_feed/convertus_vms/page_text. aprCheck is computed server-side and STORED, so a replayed cache would carry the old silence.  // 18b: the odometer is finally read as a demo signal. A 2026 Defender advertised as "New" at 6,675 km carried no demo flag, no sale_class and no hint, so deriveSaleCondition returned "new" -- and "new" drives the new-only dealer-fee ceiling and a warranty clock the report assumes has not started. condition.ts never consulted kilometres at all while msrp-basis.ts kept its own private DELIVERY_KM, so "is this car new" had two authors that could not agree; there is now one exported constant. Both production callers were also passing only 2 of the 5 inputs -- isDemo and saleClass were never handed over -- so demo detection that existed and was unit-tested could not fire outside Convertus. The threshold is MEASURED: 8,281 odometer readings across 221 Alberta year/make/model combinations of dealer-labelled new listings, p50 10 km, 93.3% at or under 100 km, a valley at 501-1000 km holding 0.3%, a second population above 2,000 km. saleCondition is computed server-side and STORED, and the CPO premium and the certified counter-script move both key off it, so a replayed cache would keep calling a driven car new.  // 18a: the report carries the CAR. A listing that publishes a photo of the vehicle on its own schema.org node now has that photo printed on page 1 of the emailed PDF, and canonical v14 seals the URL as `ph` beside the VIN it was published against. THE BUMP IS MANDATORY, not cosmetic: report-auth.ts recomputes canonicalReport() from the submitted body, so a cached analysis replayed with a v13 signature would be refused as signature_mismatch and EVERY email would fail silently for the cache TTL. Measured on the 41 captured pages: 23 publish such a photo, all 23 also publish a VIN on that same node, and 8 of the 41 declare more than one vehicle -- so the photo only travels attached to a VIN, and only prints when that VIN is the report's own. The 18 pages that publish no vehicle node print "NO PHOTO PUBLISHED"; 10 of those do publish an og:image and in all ten it is the dealer's own logo, which is why og:image is not a fallback. // 13q: freight and PDI reaches the report. Point 03 now states what the listing charges to deliver the car against what the manufacturer publishes for that model, and a listing above the published figure RAISES the point instead of rendering TRANSPARENT -- BMW Canada publishes $3,470 for a 2026 X3 in Alberta while a Calgary listing for the same car at the same $60,400 MSRP charges $4,395. The copy states both figures and asks; it never asserts the dealer added anything, because we once told a buyer to demand removal of a $3,330 line that was 70% manufacturer freight and government levies. BMW joins the freight catalogue at the Alberta-confirmed figure, which is now the highest we hold. // 13p: six makes gain a captured Freight and PDI figure (Porsche $2,950, Polestar $2,800, Acura $2,595, Lincoln $2,595, Mitsubishi $2,125, Honda $2,000), each read off an official Canadian page and confirmed by a second independent read. freightFor() and hasBrandFees() now answer for makes that previously returned null, so any surface reading them changes what it says and a replayed cache would show the old silence. BMW is deliberately NOT among them: its published figure is a CEILING ("up to $2,955") quoted from an Ontario-rendered disclaimer, while real listings run $2,995 Montreal / $3,380 Aurora / $4,395 Calgary -- subtracting a fixed $2,955 from an all-in price that contains $4,395 would bill the difference to the dealer as markup. // 13o: a price is no longer compared across two bases on ANY surface. An Alberta all-in advertised price measured against the ex-freight MSRP we hold printed +$3,164 OVER on a car nobody had marked up -- roughly $3,000 of that is Toyota's own freight and Alberta's own levies, which sit INSIDE an advertised price by law. The hero band, the on-screen gap bar and counter-script move S14 each computed it raw; only msrp-claim.ts refused. THE BUMP IS FOR S14: counterScript.moves is computed server-side and STORED in the analysis, so a replayed cache would keep reciting that accusation to a named dealer even after the fix deployed. A reference with no recorded price_basis, or one that already includes freight, now carries no subtraction either -- 854 of 1,497 catalogue rows are in that state. // 13n: the comps fetch falls back to the NAMEPLATE when the exact model matches nothing, so a car filed as "NX 350" is finally compared against the rows the crawl files under "NX" -- a real report said "Other listings read: None read" while 95 Alberta NX listings sat unread, 51 of them gas NX 350s from $54,830 to $72,146 against a car asking $72,241. The powertrain wall now reads the model the POOL was fetched under rather than the subject's, because prepending the subject's model injected its own marker into every row and would have passed the whole gas market into a hybrid's set. Both change what the market cards SAY, so a replayed cache would show the old "none". // 13l: the AMVIC lookup stops searching on the dealer's CITY and stops letting hundreds of name matches crowd the domain match out of a capped read -- "Lexus of Edmonton" resolved to CITY OF EDMONTON's licence on a signed report. // 13k: the report leads with what it found in DOLLARS instead of a 0-10 negotiation-leverage gauge, on screen and in the PDF; canonical v13 seals that total as lvd. // 13j: the model-written verdict stops sending a buyer to ask the dealer for an itemisation we already hold, and stops using US paperwork terms ("title/tags", "DMV") in a Canadian report. // 13i: a NEW vehicle with no published VIN is told what the VIN would actually settle -- which build is being sold -- instead of a used car's recall-history reason, and the price card names that same VIN as the one ask that would let it compare. // 13h: the Price-vs-MSRP card no longer calls a DEALER-STATED figure one we hold, and offers the published starting price when we have it -- but it only SUBTRACTS that figure from the asking price when the two are on the same basis, because an all-in ask minus an ex-freight MSRP counts about $3,000 of mandatory fees as markup. // 13g: a dealer fee sitting EXACTLY on the manufacturer published maximum now says so, instead of saying nothing and leaving the summary to invent a range -- and the sentence that names it only calls a figure the brand's OWN published maximum when the record is brand-level, naming the region where the figure is region-specific. // 13f: a bare schema.org Product is no longer read as a vehicle, so a Build and Price configurator stops producing a vehicle report. // 13e: Toyota's dealer-fee ceiling is region-published, not one national number -- BC & Yukon resolve to Toyota's own $990 instead of the Prairies' $999, so a BC fee between the two now reads differently. // 13d: canonical v12 seals the VIN check digit beside the VIN. validateVin computed it on every scan and only admin telemetry ever saw it; /verify printed the VIN as a bare fact. // 13c: "price verified" is STRICT wherever it is a CLAIM; VIN shape decided once in _shared/vin.ts. // 13b: a cached place_id can now resolve a dealer that name+city could not, so point 10 can say something where it previously said nothing. Same-day as 13a, so the practical cost is one invalidation, not two.  // 13a: point 10 stops attaching another company's reviews to this dealer. Places identity is verified against the listing's own domain or name+city instead of taking searchData.places[0], and a Places OUTAGE no longer renders as "NONE FOUND -- we searched and found no public reviews". Both change what the card says, so a replayed cache would show the old wrong answer.  // 12e: the AMVIC website path is finally WIRED -- it shipped in 12c with the matcher built and unit-tested and the caller never passing it the domains, so every licence card that depended on it still read "No dealer name was confirmed".  // 12d: all four of the dealer platform's own listing dates are read, not just date_on_lot -- date_updated (is the asking price current or months stale) and date_sold (a sale recorded on a still-live listing) are new report lines. // 13m: BEFORE YOU SIGN -- the report gained a phone-first desk card. No new claims, so a replayed cache is not wrong, but the surface is new and the bump keeps the cache honest about what a stored analysis can render. // 13l: the AMVIC lookup stops searching on the dealer's CITY and stops letting hundreds of name matches crowd the domain match out of a capped read -- "Lexus of Edmonton" resolved to CITY OF EDMONTON's licence on a signed report. // 13k: the report leads with what it found in DOLLARS instead of a 0-10 negotiation-leverage gauge, on screen and in the PDF; canonical v13 seals that total as lvd. // 13j: the model-written verdict stops sending a buyer to ask the dealer for an itemisation we already hold, and stops using US paperwork terms ("title/tags", "DMV") in a Canadian report. // 13i: a NEW vehicle with no published VIN is told what the VIN would actually settle -- which build is being sold -- instead of a used car's recall-history reason, and the price card names that same VIN as the one ask that would let it compare. // 13h: the Price-vs-MSRP card no longer calls a DEALER-STATED figure one we hold, and offers the published starting price when we have it -- but it only SUBTRACTS that figure from the asking price when the two are on the same basis, because an all-in ask minus an ex-freight MSRP counts about $3,000 of mandatory fees as markup. // 13g: a dealer fee sitting EXACTLY on the manufacturer published maximum now says so, instead of saying nothing and leaving the summary to invent a range -- and the sentence that names it only calls a figure the brand's OWN published maximum when the record is brand-level, naming the region where the figure is region-specific. // 13f: a bare schema.org Product is no longer read as a vehicle, so a Build and Price configurator stops producing a vehicle report. // 13e: Toyota's dealer-fee ceiling is region-published, not one national number -- BC & Yukon resolve to Toyota's own $990 instead of the Prairies' $999, so a BC fee between the two now reads differently. // 13d: canonical v12 seals the VIN check digit beside the VIN. validateVin computed it on every scan and only admin telemetry ever saw it; /verify printed the VIN as a bare fact. // 13c: "price verified" is STRICT wherever it is a CLAIM; VIN shape decided once in _shared/vin.ts. // 13b: a cached place_id can now resolve a dealer that name+city could not, so point 10 can say something where it previously said nothing. Same-day as 13a, so the practical cost is one invalidation, not two.  // 13a: point 10 stops attaching another company's reviews to this dealer. Places identity is verified against the listing's own domain or name+city instead of taking searchData.places[0], and a Places OUTAGE no longer renders as "NONE FOUND -- we searched and found no public reviews". Both change what the card says, so a replayed cache would show the old wrong answer.  // 12e: the AMVIC website path is finally WIRED -- it shipped in 12c with the matcher built and unit-tested and the caller never passing it the domains, so every licence card that depended on it still read "No dealer name was confirmed".  // 12d: all four of the dealer platform's own listing dates are read, not just date_on_lot -- date_updated (is the asking price current or months stale) and date_sold (a sale recorded on a still-live listing) are new report lines.
...
async function buildJsonLdFallbackAnalysis(url: string, sharedHtml?: Promise<string | null>): Promise<any | null> {
  try {
    ...
    const analysis: any = {
      ...
      // The listing's own photo of this car and the VIN it is anchored to,
      // both read off the same schema.org node. They travel as a pair; the
      // PDF refuses to draw one without the other.
      vehiclePhotoUrl: v.vehiclePhotoUrl ?? null, vehiclePhotoVin: v.vehiclePhotoVin ?? null,
...
async function captureMarketCount(analysis: any, urlHint?: string | null): Promise<void> {
  try {
    ...
    const fetchComps = (m: string) => {
      const rpc = supabase.rpc("fn_market_comps", {
        p_year: year, p_make: make, p_model: m, p_condition: condition,
      ...
      return Promise.race([rpc, timeout]) as any;
    };
    const { data, error } = await fetchComps(model);
    ...
    // THE SECOND FETCH, WIDENED TO THE NAMEPLATE.
    //
    // fn_market_comps matches the model by exact string equality, and the two
    // sides do not spell it the same way. A subject page parses as model
    // "NX 350"; the crawled listings are stored as model "NX" with "NX 350" in
    // the trim, because that is how the dealer pages name them. On 2026-09-16 a
    // real report told a buyer "None read" while we held 95 Alberta NX listings,
    // 51 of them gas NX 350s from $54,830 to $72,146, against a car asking
    // $72,241 -- the top of the range, and the comparison that mattered most.
    //
    // This runs ONLY when the exact match found nothing, so it cannot change a
    // result that already works. Widening cannot blur a powertrain either:
    // everything it returns still goes through the same wall below, which
    // rejects a hybrid row for a gas subject on the model AND the trim string.
    if (!rows.length) {
      const wide = baseNameplate(model);
      if (wide) {
        const second = await fetchComps(wide);
        if (second?.error) console.warn("market count: nameplate retry failed:", second.error.message);
        else if (Array.isArray(second?.data)) {
          rows = second.data;
          if (rows.length) console.log(`market count: "${model}" matched nothing, "${wide}" matched ${rows.length}`);
        }
      }
    }
...
async function enrichAnalysisInner(analysis: any, deadline?: number): Promise<void> {
  ...
  // THE RATE CHECK. Is the quoted rate above a published one, and what does the
  // difference cost across the whole loan? computeFinancingTrap answers a
  // different question (is a discount worth more than the rate it is contingent
  // on); this answers "am I being charged above the advertised rate", which is
  // the 2026-08-19 shape done correctly.
  //
  // THE BENCHMARK IS THE MANUFACTURER'S OWN PUBLISHED RATE, and only where the
  // posted TERM matches the loan's term. A rate posted for 60 months says
  // nothing about an 84-month loan, so most deals will refuse here rather than
  // compare across terms -- the same discipline as an all-in price against an
  // ex-freight MSRP. An independent lender benchmark would cover the rest;
  // Canadian retail lenders do not publish auto rates (13 pages checked
  // 2026-09-22, see docs/FIXING-HISTORY.md), so that source does not exist yet.
  const mfr = analysis?.financeRates?.manufacturer;
  const benchmark: AprBenchmark | null = (mfr && mfr.apr != null && mfr.termMonths && mfr.effectiveDate)
    ? {
      apr: Number(mfr.apr),
      lender: `${analysis.make || "the manufacturer"}`,
      capturedOn: String(mfr.effectiveDate),
      termMin: Number(mfr.termMonths),
      termMax: Number(mfr.termMonths),
      qualifier: mfr.promo ? "advertised promotional rate" : "published finance rate",
    }
    : null;
  analysis.aprCheck = aprClaim({
    // trustedDealerApr's rule, not the LLM's read: an invented rate once became
    // a quantified accusation against a named dealer.
    quotedApr: TRUSTED_APR_SOURCES.has(analysis?.financeRates?.dealer?.source)
      ? analysis?.financeRates?.dealer?.apr ?? null
      : null,
    benchmark,
    loan: analysis?.quotedPrice ?? null,
    termMonths: analysis?.financing?.termMonths ?? null,
  ...
  {
    // ONE resolution for both paths. This used to be a city-only
    // resolveAllInAuthority with a three-state fallback here, and analyze-quote
    // had the city-only half WITHOUT the fallback — so "we could not tell"
    // rendered there as "not an all-in province" and the subtraction ran. Two
    // copies of one rule, and only one of them carried the fix.
...
const CORS_HEADERS = {
  ...
  // GET is declared because this function answers one: the deployed-version
  // readback below. It carries no body and reads no input.
...
Deno.serve(async (req: Request) => {
  ...
  // WHICH BUNDLE IS ACTUALLY RUNNING HERE? A GET answers with this bundle's own
  // CACHE_VER, and with nothing else.
  //
  // WHY THIS EXISTS. deploy-edge-functions.yml ran `supabase functions deploy`,
  // read the exit code, and reported success. Exit 0 says the CLI finished. It
  // does not say which bundle the gateway is now serving, and nothing in the
  // workflow ever asked. Every correctness fix listed against CACHE_VER above
  // is keyed on it -- `_cacheVer !== CACHE_VER` is the ONLY thing that makes a
  // stored analysis re-scan -- so a deploy that silently shipped an older
  // bundle would leave every cached report replaying the pre-fix claim while
  // the workflow showed green. That is "Green signal, no check", the first
  // shape named at the top of docs/FIXING-HISTORY.md, sitting on the step that
  // puts code in front of buyers. [[verify-in-production-before-done]]
  //
  // WHAT IT COSTS AND WHAT IT EXPOSES: nothing, by construction.
  //   - GET only. The body is never read and the query string is never
  //     consulted, so there is no way to hand this branch a URL to scan.
  //   - No vendor call, no credit hold, no database round trip -- it returns
  //     before resolveCreditUser and before every fetch in this file, so it
  //     cannot spend a cent at Scrapfly, Nimble or Anthropic. [[cost-exploit-guards]]
  //   - The response is one constant string that is already public in this
  //     repo. It names no key, no customer, no listing, no environment value.
  //
  // IT RETURNS BEFORE THE VENDOR-KEY CHECK DELIBERATELY. The moment you most
  // need to know which bundle is live is the moment the function is
  // misconfigured; a readback that goes dark exactly then answers nothing.
  if (req.method === "GET") {
    return new Response(
      JSON.stringify({ cacheVer: CACHE_VER }),
      {
        status: 200,
  ...
  try {
    ...
    try {
      ...
      if (early) {
        for (const k of ["quotedPrice", "vin", "odometerKm", "dealerName", "dealerCity", "vehicleCondition", "fuelType", "vehiclePhotoUrl", "vehiclePhotoVin"] as const) {
          const cur = (analysis as any)[k];
