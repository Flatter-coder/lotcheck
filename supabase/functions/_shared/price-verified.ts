 ...
 */
/**
 * "May this surface CLAIM the price was verified?" — the strict question, asked
 * across both shapes.
 *
 * The mirror of priceUsableForComparison: that one asks whether a price exists
 * to compute from, this one asks whether we may say we checked it. Every
 * surface that prints the word "verified" takes this; every surface that gates
 * a computation takes the other. Keeping them apart is the point.
 */
export function priceVerifiedClaim(a: any): boolean {
  // The sealed record already carries the strict verdict.
  const sealed = a?.price?.verified;
  if (typeof sealed === "boolean") return sealed;
  return resolvePriceVerified(a).sourceVerified;
}
