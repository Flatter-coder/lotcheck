...
function resolveEvap(a){
  ...
  return {
    ...
    rebate: getRebate("AB", effectiveFuelType, {
      ...
      km:a.odometerKm ?? null,   // a car with no published odometer has not driven 0 km [[read-num]]
...
function encodeReport(a){
  const c={v:a.vehicle,y:a.year,mk:a.make,md:a.model,tr:a.trim,dn:a.dealerName,dc:a.dealerCity,cond:a.vehicleCondition,
    ...
    // WHETHER WE COULD LOOK is its own fact, and it never crossed the share
    // link -- so a forwarded copy of a report that DID read the pricing area
    // reported "NOT READ". Absent on a legacy link decodes to undefined, which
    // keeps old links on today's conservative answer rather than upgrading one
    // to a clean bill it never earned. [[missing-beats-wrong]]
    frd:a.feesRead===true?1:0,
    ...
    fm:a.financingCheck?{r:!!a.financingCheck.consistent,n:a.financingCheck.note||null,ck:a.financingCheck.checked===true?1:0,p:a.financingCheck.paymentsCounted??null,cp:a.financingCheck.computedFromPayments??null,dt:a.financingCheck.disclosedTotalObligation??null}:null};
...
function decodeReport(s){
  try{
    ...
    return {vehicle:c.v,year:c.y,make:c.mk,model:c.md,trim:c.tr,dealerName:c.dn,dealerCity:c.dc,vehicleCondition:c.cond,
      ...
      odometerKm:c.odo!=null&&Number.isFinite(Number(c.odo))?Number(c.odo):null,
      ...
      feesRead:c.frd===1?true:undefined,
      sourceUrl:c.src?(c.src.u||null):null,
      capturedAt:c.src?(c.src.c||null):null,
      financingCheck:c.fm?(c.fm.ck===undefined?{note:c.fm.n||null}:{checked:c.fm.ck===1,consistent:!!c.fm.r,note:c.fm.n||null,paymentsCounted:c.fm.p??null,computedFromPayments:c.fm.cp??null,disclosedTotalObligation:c.fm.dt??null}):null,
      __shared:true};
  }catch{ return null; }
}
...
function canonicalReport(a){
  ...
  return {
    ...
    // v15 (2026-09-24): marketValue.rows, the sealed listings. Mirrors report-sign.ts.
    // v16 (2026-09-25): marketValue.pool + ot, the shortlist rows. Mirrors report-sign.ts.
    v:16,
              ...
              return (<div>
                ...
                <div style={{background:vdark?"rgba(255,255,255,.04)":"rgba(255,255,255,.6)",border:`1px solid ${T.cardBd}`,borderRadius:12,padding:"14px 16px"}}>
                  ...
                  {/* A rating "over 0 reviews" names nothing that was read, and
                      ...
                      it is real. No count, no count. [[read-num]] */}
                  {o.reputation&&<Row t="Dealer reputation" v={`${Number(o.reputation.rating).toFixed(1)}★${o.reputation.reviews==null?"":` · ${Number(o.reputation.reviews).toLocaleString()} reviews`}`}/>}
        ...
        <div style={{maxWidth:wOn?"none":(status==="idle"?820:640),margin:"0 auto",padding:wOn?0:"24px 16px"}}>
          <div style={{marginBottom:wOn?0:24}}>
            {/* The wizard's own step-1 question ("How do you want to check
                ...
                error) where there's no step heading doing that job. "Works
                with New/Demo/Certified/Used" and the marketplace-only note
                (which used to live here) moved into the wizard itself --
                the marketplace note is now on the "link" step, where it's
                actually relevant. */}
            {status!=="idle"&&(
              <>
          ...
          <div>
            ...
            {tiers?(
              <>
                ...
                <div className="vr-place">{a.topEnd?<>With no accidents and full records, you sit at the <b>top</b> of each range — about {money(tiers.trade.point)} on a trade, {money(tiers.privateParty.point)} selling it privately.</>:<>About {money(tiers.trade.point)} on a trade, or {money(tiers.privateParty.point)} selling it privately.</>}<span className="vr-faint2">{(mv&&mv.comps)||0} Alberta comp{(mv&&mv.comps)===1?"":"s"} · {a.adjusted?"read at your mileage":"asking price, not mileage-adjusted"}{a.mileageAdj&&a.mileageAdj.extrapolated?" · your mileage is outside our current comps — indicative":""} · not sold data</span></div>
              </>
            ):mv&&mv.average!=null&&(
            ...
            <div className="vr-card">
              ...
              {tiers&&(
                <div className="vr-row"><div className="k">How you land</div><div className="v">{a.topEnd?<>A clean, no-accident history holds you at the <b style={{color:"#fff"}}>top</b> of each range and answers the "high&nbsp;km" worry before a buyer asks.</>:(a.accidents!=="unknown"||a.serviceHistory!=="unknown")?<>Based on what you told us you sit mid-range — a clean, no-accident history with full records would move you to the <b style={{color:"#fff"}}>top</b>. The full PDF shows every lever.</>:<>Tell us your accident &amp; service history above for a personalized placement. The full PDF shows every lever.</>}</div></div>
              )}
              {rc&&(
